...
if(isset($params['submit']))
{
	...
	if(isset($params['sel']) && $params['sel'] != '')
	{
		...
		$item_fournisseur = '';
		if(isset($params['item_fournisseur']) && $params['item_fournisseur'] !='')
		{
			$item_fournisseur = $params['item_fournisseur'];
		}
		...
		$i = 0;//on instancie un compteur pour rendre compte
			...
			foreach($a as $valeur)
			{
				...
				$query = "UPDATE ".cms_db_prefix()."module_commandes_items SET fournisseur = ? WHERE id = ? ";
				
				$dbresult = $db->Execute($query, array($item_fournisseur,$valeur));

				if($dbresult)
				// && $dbresult->RecordCount()>0)
				{
					$designation.= "Ok->Fournisseur changé ";
...
else
{
	...
	{			
		...
		$smarty->assign('formstart',
				    $this->CreateFormStart( $id, 'change_item_fournisseur', $returnid ) );	
		$smarty->assign('record_id',
				$this->CreateInputHidden($id,'sel',$sel));
		
	
		$smarty->assign('item_fournisseur',
				$this->CreateInputDropdown($id,'item_fournisseur',$liste_fournisseurs));//,$selectedIndex=$key2_statut_commande,$selectedvalue=$statut_commande));
		...
		echo $this->ProcessTemplate('change_item_fournisseur.tpl');
	}
}
