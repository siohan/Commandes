...
if(isset($params['submit']))
{
	...
	if(isset($params['sel']) && $params['sel'] != '')
	{
		...
		$item_prix = '';
		if(isset($params['item_prix']) && $params['item_prix'] !='')
		{
			$item_prix = $params['item_prix'];
		}
		...
		$i = 0;//on instancie un compteur pour rendre compte
			...
			foreach($a as $valeur)
			{
				...
				$query = "UPDATE ".cms_db_prefix()."module_commandes_items SET prix_unitaire = ? WHERE id = ? ";
				
				$dbresult = $db->Execute($query, array($item_prix,$valeur));

				if($dbresult)
				// && $dbresult->RecordCount()>0)
				{
					$designation.= "Ok->Prix changé ";
...
else
{
	...
	{			
		...
		$smarty->assign('formstart',
				    $this->CreateFormStart( $id, 'change_item_prix', $returnid ) );	
		$smarty->assign('record_id',
				$this->CreateInputHidden($id,'sel',$sel));
		
	
		$smarty->assign('item_prix',
				$this->CreateInputText($id,'item_prix','',50, 150));//,$selectedIndex=$key2_statut_commande,$selectedvalue=$statut_commande));
		...
		echo $this->ProcessTemplate('change_item_prix.tpl');
	}
}
