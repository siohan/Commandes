...
if(isset($params['submit']))
{
	...
	if(isset($params['sel']) && $params['sel'] != '')
	{
		...
		$item_reduction = '';
		if(isset($params['item_reduction']) && $params['item_reduction'] !='')
		{
			$item_reduction = $params['item_reduction'];
		}
		...
		$i = 0;//on instancie un compteur pour rendre compte
			...
			foreach($a as $valeur)
			{
				...
				$query = "UPDATE ".cms_db_prefix()."module_commandes_items SET reduction = ? WHERE id = ? ";
				
				$dbresult = $db->Execute($query, array($item_reduction,$valeur));

				if($dbresult)
				// && $dbresult->RecordCount()>0)
				{
					$designation.= "Ok->Réduction changée ";
...
else
{
	...
	{			
		...
		$smarty->assign('formstart',
				    $this->CreateFormStart( $id, 'change_item_reduction', $returnid ) );	
		$smarty->assign('record_id',
				$this->CreateInputHidden($id,'sel',$sel));
		
	
		$smarty->assign('item_reduction',
				$this->CreateInputText($id,'item_reduction','',50, 150));//,$selectedIndex=$key2_statut_commande,$selectedvalue=$statut_commande));
		...
		echo $this->ProcessTemplate('change_item_reduction.tpl');
	}
}
